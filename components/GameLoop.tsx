...
import { Pause, Play, LogOut, Maximize, Minimize, HelpCircle } from 'lucide-react';
...
const GameLoop: React.FC<Props> = ({ character, onExit, onDeath, presetEnemy, onRest }) => {
  ...
  return (
    /* === WIREFRAME LAYOUT ===
       Preto (outer): dinâmico, absorve diferenças de tela
       Cinza (frame): medieval-900, contém canvas + overlays de UI
    ...
    */
    <div className="fixed inset-0 bg-black flex items-start justify-center overflow-hidden select-none touch-none safe-all">
      ...
      <div className="relative w-full h-full bg-medieval-900 flex items-start justify-center">
        ...
        {/* === OVERLAY: fullscreen + pause (topo, sobre o canvas) === */}
        <div className="absolute top-0 left-0 right-0 flex justify-between items-center p-2 z-30 pointer-events-none">
          <button
            onClick={toggleFullscreen}
            className="pointer-events-auto bg-medieval-700 border-2 border-medieval-500 rounded-full flex items-center justify-center active:scale-90 transition-transform shadow-lg touch-none"
        ...
        {/* === OVERLAY: items usáveis (lado direito, sobre o canvas) === */}
        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-1.5 z-30">
          ...
          >
            {hudStatic.equippedUsable1 ? (hudStatic.equippedUsable1.icon ? renderIcon(hudStatic.equippedUsable1.icon, 22, 'text-red-400') : <HelpCircle size={22} className="text-gray-500" />) : <HelpCircle size={22} className="text-gray-500" />}
          ...
          >
            {hudStatic.equippedUsable2 ? (hudStatic.equippedUsable2.icon ? renderIcon(hudStatic.equippedUsable2.icon, 22, 'text-red-400') : <HelpCircle size={22} className="text-gray-500" />) : <HelpCircle size={22} className="text-gray-500" />}
        ...
        {/* === OVERLAY: joystick + HP bars + abilities + attack (base, sobre o canvas) === */}
        <BottomControls
          character={character}
        ...
        {isPaused && !battleSummary?.show && (
          <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-40">
            <div className="bg-medieval-800 border-4 border-medieval-500 rounded-lg shadow-2xl text-center p-8" style={{ width: 'min(90%, 360px)' }}>
              <h2 className="font-serif mb-6 text-medieval-200 text-3xl">Paused</h2>
              <div className="flex flex-col gap-3">
                <button onClick={togglePause} className="w-full py-3 bg-emerald-800 hover:bg-emerald-700 text-white font-bold rounded flex items-center justify-center gap-2 border border-emerald-600 active:scale-95 transition-transform">
                  <Play size={18} fill="currentColor" /> Resume
                </button>
                <button onClick={handleQuitToMap} className="w-full py-3 bg-red-900 hover:bg-red-800 text-white font-bold rounded flex items-center justify-center gap-2 border border-red-700 active:scale-95 transition-transform">
                  <LogOut size={18} /> Quit to Map
                </button>
              </div>
              <p className="mt-4 text-medieval-500 text-xs">Quitting counts as fleeing (XP penalty if no Bravery).</p>
            </div>
          </div>
        )}
